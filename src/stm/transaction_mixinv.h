...
public:
	// possible CM phases
	enum CmPhase {
		CM_PHASE_INITIAL, CM_PHASE_GREEDY
	};
	...
	enum TxStatus {
		TX_IDLE, TX_EXECUTING, TX_ABORTED, TX_RESTARTED, TX_COMMITTED
	};

	//TLSTM
	struct ProgramThread {

		union CACHE_LINE_ALIGNED {
			volatile Word valid_ts;
			char padding_valid_ts[CACHE_LINE_SIZE_BYTES];
		};
		...
		union CACHE_LINE_ALIGNED {
			...
			char padding_validated[CACHE_LINE_SIZE_BYTES];
		};

		union CACHE_LINE_ALIGNED {
			volatile bool requires_validation;
			char padding_requires_validation[CACHE_LINE_SIZE_BYTES];
		};

		union CACHE_LINE_ALIGNED {
			volatile bool is_alive;
			char padding_is_alive[CACHE_LINE_SIZE_BYTES];
		};
...
public:
	...
	static void InitializeProgramThreads(Word nb_threads);
	...
	void InconsistentReadRestart();

	void TxRestartNoJMP(RestartCause cause =
				RESTART_EXTERNAL);
	/**
	 * Rollback transaction's effects and jump to the beginning with flag specifying restart.
	 */
	void TxRestart(RestartCause cause =
			RESTART_EXTERNAL);

	void WriteWord(Word *address, Word val,
			Word mask = LOG_ENTRY_UNMASKED);
	...
	void LockMemoryBlock(void *address,
			size_t size);
...
private:
	...
	bool address_is_invalid(Word *address);
	...
	static bool ValidateTx(int ptid);
...
private:
	...
	// w shared
	union CACHE_LINE_ALIGNED {
		volatile bool abort_task;
		char padding_abort_task[CACHE_LINE_SIZE_BYTES];
	};

	// w shared
	union CACHE_LINE_ALIGNED {
		volatile bool aborting_transaction;
		char padding_aborting_transaction[CACHE_LINE_SIZE_BYTES];
	};
...
private:
	...
	static pthread_t threadid[MAX_THREADS];
...
inline void* validate_transaction(void* data) {

	Word ptid = (Word)data;

	sem_init(&tlstm::TxMixinv::prog_thread[ptid].semaphore, 0, 0);
	tlstm::TxMixinv::prog_thread[ptid].valid_ts = tlstm::TxMixinv::commit_ts.readCurrentTsAcquire();

	for (int i = 0; i < tlstm::TxMixinv::specdepth; i++) {
		while (!atomic_load_acquire(&tlstm::TxMixinv::prog_thread[ptid].owners[i]));
	}
	...
	while(true){
		...
		if(tlstm::TxMixinv::prog_thread[ptid].requires_validation == true){
			tlstm::TxMixinv::prog_thread[ptid].validated = false;
			tlstm::TxMixinv::prog_thread[ptid].requires_validation = false;
			if(!tlstm::TxMixinv::ExtendTx(ptid)){
				for (int i = 0; i < tlstm::TxMixinv::specdepth; i++) {
...
inline void tlstm::TxMixinv::GlobalInit(int nb_tasks, int nb_threads) {
	...
	InitializeProgramThreads(nb_tasks * nb_threads);
	...
	for(int k=0; k < nb_threads; k++){
		//prog_thread[i].is_alive = true;

		pthread_attr_init(&attr);
		pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

		CPU_ZERO(&cpuset);
		CPU_SET(nb_tasks*nb_threads+k, &cpuset);
		pthread_attr_setaffinity_np(&attr, sizeof(cpuset), &cpuset);

		pthread_create(&threadid[k], &attr, validate_transaction, (void *)k);
	}
...
inline void tlstm::TxMixinv::SigSEGVHandler(int sig){
	for(Word i=0; i < MAX_THREADS; i++){
		if(transactions[i]->posixtid == pthread_self()){
			siglongjmp(transactions[i]->start_buf, LONG_JMP_INCONSISTENT_READ_FLAG);
		}
	}
}
...
//TLSTM
inline void tlstm::TxMixinv::InitializeProgramThreads(Word nb_threads) {

	for (Word i = 0; i < nb_threads; i++) {
		...
		prog_thread[i].validated = true;
		prog_thread[i].requires_validation = false;
	}
}
...
inline void tlstm::TxMixinv::ThreadShutdown() {
	//atomic_store_release(&prog_thread[prog_thread_id].is_alive, false);
	//prog_thread[prog_thread_id].is_alive = false;
	//pthread_cancel(threadid);
	//pthread_kill(threadid[pro], 9);
}
...
inline void tlstm::TxMixinv::TxStart(int lex_tx_id, int new_serial, int start_s, int commit_s) {
#ifdef PERFORMANCE_COUNTING
...
#elif defined PRIVATIZATION_QUIESCENCE_TREE
	privatization_tree.setNonMinimumTs(prog_thread[prog_thread_id].valid_ts);
#endif /* PRIVATIZATION_QUIESCENCE */

#ifdef MM_EPOCH
	UpdateLastObservedTs(prog_thread[prog_thread_id].valid_ts);
#endif /* MM_EPOCH */
...
inline void tlstm::TxMixinv::TxCommit() {
	...
	*/
	...
	if (try_commit) {
		...
		}while (prog_thread[prog_thread_id].requires_validation == true || prog_thread[prog_thread_id].validated == false);
	}

	//either an intermediate task needs to abort because of a WW conflict
	//or the commit task needs to abort because of TM validation
...
inline tlstm::TxMixinv::RestartCause tlstm::TxMixinv::TxTryCommit(bool read_only) {
	Word ts = prog_thread[prog_thread_id].valid_ts;
	...
	if (!try_commit) {
		...
		if (!read_only || prog_thread[prog_thread_id].last_completed_writer >= start_serial) {
...
#ifdef COMMIT_TS_INC
			if(ts != prog_thread[prog_thread_id].valid_ts + 1){
			...
			//warn previous tasks of successful commit
			prog_thread[prog_thread_id].last_completed_task = serial;
			//atomic_store_release(&prog_thread[prog_thread_id].last_completed_task, serial);
		}
	...
	inline void tlstm::TxMixinv::Rollback(RestartCause cause) {
		if (rolled_back) {
			return;
		}
	...
	inline void tlstm::TxMixinv::RollbackTransaction(Word start_s) {
		...
		for (Word i = start_s; i <= serial; i++) {
			prog_thread[prog_thread_id].owners[i % specdepth]->abort_transaction = false;
		}

		prog_thread[prog_thread_id].requires_validation = false;
		prog_thread[prog_thread_id].validated = true;
		prog_thread[prog_thread_id].valid_ts = commit_ts.readCurrentTsAcquire();

		for (Word i = start_s; i < serial; i++) {
			prog_thread[prog_thread_id].owners[i % specdepth]->aborting_transaction = true;
		}
	}

	inline void tlstm::TxMixinv::ReadyToAbortTransaction() {
		prog_thread[prog_thread_id].owners[commit_serial % specdepth]->aborting_transaction = true;
	...
	inline void tlstm::TxMixinv::LockMemoryStripe(WriteLock *write_lock, Word *address, Word value, Word mask) {
			...
			if (locked) {
				if (entry->ptid != prog_thread_id) {
					...
					if (entry->serial < serial) {
						...
						prog_thread[prog_thread_id].owners[entry->serial % specdepth]->abort_task = true;
		...
		if (ShouldExtend(version)) {
			prog_thread[prog_thread_id].requires_validation = true;
			//sem_post(&prog_thread[prog_thread_id].semaphore);
		}
	...
	inline Word tlstm::TxMixinv::ReadWordInner(Word *address) {
		...
		if (address_is_invalid(address)) {
			if (prog_thread[prog_thread_id].validated == false || abort_transaction) {
				if (try_commit) {
					while (prog_thread[prog_thread_id].last_completed_task != serial - 1);
...
*/
	...
	inline bool tlstm::TxMixinv::address_is_invalid(Word *address) {
	...
	inline bool tlstm::TxMixinv::ShouldExtend(VersionLock version) {
		if (get_value(version) > prog_thread[prog_thread_id].valid_ts) {
			return true;
		}
	...
	inline bool tlstm::TxMixinv::ValidateTx(int ptid) {
		ReadLog::iterator iter;

		for (int i = 0; i < specdepth; i++) {
			for (iter = prog_thread[ptid].owners[i]->read_log.begin(); iter.hasNext(); iter.next()) {
				ReadLogEntry &entry = *iter;
				VersionLock currentVersion = (VersionLock) atomic_load_no_barrier(entry.read_lock);
	...
	inline bool tlstm::TxMixinv::ValidateTLS() {
		for (TaskReadLog::iterator iter = task_read_log.begin(); iter.hasNext(); iter.next()) {
			...
			if (log_entry == NULL || entry.serial != log_entry->serial) {
				return false;
			}
		}
	...
	inline Word tlstm::TxMixinv::ValidateCommit() {
		ReadLog::iterator iter;
	...
	inline bool tlstm::TxMixinv::Extend() {
		...
		if (Validate()) {
			prog_thread[prog_thread_id].valid_ts = ts;
#ifdef PRIVATIZATION_QUIESCENCE
			*quiescence_ts = ts;
#elif defined PRIVATIZATION_QUIESCENCE_TREE
	...
	inline bool tlstm::TxMixinv::ExtendTx(int ptid) {
		unsigned ts = commit_ts.readCurrentTsAcquire();

		if (tlstm::TxMixinv::ValidateTx(ptid)) {
			prog_thread[ptid].valid_ts = ts;

#ifdef TS_EXTEND_STATS
			prog_thread[ptid].owners[0]->stats.IncrementStatistics(Statistics::EXTEND_SUCCESS);
#endif /* TS_EXTEND_STATS */
			return true;
		}

#ifdef TS_EXTEND_STATS
		prog_thread[ptid].owners[0]->stats.IncrementStatistics(Statistics::EXTEND_FAILURE);
#endif /* TS_EXTEND_STATS */
	...
	inline void tlstm::TxMixinv::InconsistentReadRestart() {
		if (prog_thread[prog_thread_id].validated == false || abort_transaction) {
			if (try_commit) {
				while (prog_thread[prog_thread_id].last_completed_task != serial - 1);
		...
		++aborts;
#endif // ADAPTIVE_LOCKING
		TxRestartNoJMP(RESTART_VALIDATION);
	}

	inline void tlstm::TxMixinv::TxRestartNoJMP(RestartCause cause) {
#ifdef PRIVATIZATION_QUIESCENCE
		*quiescence_ts = MINIMUM_TS;
#elif defined PRIVATIZATION_QUIESCENCE_TREE
		privatization_tree.setNonMinimumTs(MINIMUM_TS);
		...
		if (is_write_locked(lock_value)) {
			...
			if (prog_thread[log_entry->ptid].progress > prog_thread[prog_thread_id].progress) {
				...
				for (Word i = log_entry->owner->start_serial; i <= log_entry->owner->commit_serial; i++) {
					prog_thread[log_entry->ptid].owners[i % specdepth]->abort_transaction =	true;
				}
	...
	inline bool tlstm::TxMixinv::CMStrongerThan(Word ptid) {
